...
class Builder:
    ...
    def update_version_number(self, scheme, new_version):
        ...
        try:
            ...
            version_number = version_info["MATLAB_VERSION_NUMBER"]
        ...
        # Update MatlabVersion.h
        version_info["MATLAB_VERSION_NUMBER"] = '"' + version_number + '"'
        if scheme == "production":
            version_info["MATLAB_IS_PRODUCTION_RELEASE"] = "true"
            if self.verbosity >= 1:
                print "Making a production release"
        else:
            version_info["MATLAB_IS_PRODUCTION_RELEASE"] = "false"
            if self.verbosity >= 1:
    ...
    def build_executable(self, clean_build_first, concurrency):
        ...
        if self.verbosity > 1:
            print "Building MATLAB plug-ins..."
        buildenv = os.environ
        buildenv["MATLABDEPENDENCIES"] = self.depend_path
        ...
        if self.verbosity > 1:
            print "Done building MATLAB plug-ins"

    def prep_to_run_helper(self, plugin_suffixes):
        if self.verbosity > 1:
            print "Copying Opticks plug-ins into MATLAB workspace..."
        ...
        extension_dep_file = join(extension_bin_path, "MATLAB.dep")
        if not os.path.exists(extension_dep_file):
            if self.verbosity > 1:
                print "Creating MATLAB.dep file..."
            ...
            extension_dep.close()
            if self.verbosity > 1:
                print "Done creating MATLAB.dep file"
...
class WindowsBuilder(Builder):
    ...
    def compile_code(self, env, clean, concurrency):
        solution_file = os.path.abspath("Code/MATLAB.sln")
...
def read_version_h(path=None):
    if path is None:
        version_path = join("Code", "Include", "MatlabVersion.h")
    else:
        version_path = path
...
def build_installer(aeb_platforms=[], aeb_output=None,
    ...
    manifest = dict()
    version_info = read_version_h()
    manifest["version"] = version_info["MATLAB_VERSION_NUMBER"][1:-1]
    manifest["name"] = version_info["MATLAB_NAME"][1:-1]
    manifest["description"] = version_info["MATLAB_NAME_LONG"][1:-1]
    aebl_platform_str = ""
    ...
    out_path = os.path.abspath(join("Installer","MATLAB.aeb"))
    ...
    extension_support_files_dir = join(os.path.abspath("Release"), "SupportFiles", "MATLAB")
    copy_files_in_dir_to_zip(extension_support_files_dir, join("content", "SupportFiles", "MATLAB"), zfile, [".m"], ["_svn", ".svn"])
    copy_file_to_zip(os.path.abspath("Installer"), "license", "lgpl-2.1.txt", zfile)
    doc_path = os.path.abspath(join("Code", "Build", "DoxygenOutput"))
    copy_files_in_dir_to_zip(doc_path, join("content", "Help", "MATLAB"), zfile)
    ...
    for plat in aeb_platforms:
        ...
        if plat_parts[0].startswith('win'):
            ...
            copy_file_to_zip(extension_plugin_path, target_plugin_path, "MatlabInterpreterManager.dll", zfile)
            extension_interpreter_path = join(extension_plugin_path, "MATLAB")
            target_interpreter_path = join(target_plugin_path, "MATLAB")
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2006b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2007a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2007b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2008a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2008b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2009a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2009b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2009bSP1.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2010a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2010b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2010bSP1.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2011a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2011b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2012a.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2012b.dll", zfile)
            copy_file_to_zip(extension_interpreter_path, target_interpreter_path, "MatlabInterpreterR2013a.dll", zfile)
        else:
...
def main(args):
    ...
    options.set_defaults(mode="release", clean=False,
        build_extension=False, build_doxygen=False,
    ...
    builder = None
    try:
        opticks_depends = os.environ.get("MATLABDEPENDENCIES", None)
        ...
        if not is_windows():
            raise ScriptException("This extension is only supported on Windows")
        else:
